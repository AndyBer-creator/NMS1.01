...
echo "[smoke] 1/7 health endpoint"
expect_http_200 "$BASE_URL/health" "$tmp_dir/health.txt"

echo "[smoke] 2/7 api metrics endpoint"
...
if [[ -n "${NMS_ADMIN_USER:-}" && -n "${NMS_ADMIN_PASS:-}" ]]; then
  ...
  echo "[smoke] 5/7 authenticated devices list"
  devices_code="$(
  ...
  echo "[smoke] 6/7 availability page"
  expect_http_200_cookie "$BASE_URL/events/availability/page" "$tmp_dir/availability_page.html" "$cookie_jar"
else
  echo "[smoke] 4-6/7 skipped login-dependent checks (NMS_ADMIN_USER/PASS not set)"
fi

echo "[smoke] 7/7 worker metrics endpoint"
