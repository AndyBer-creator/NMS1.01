...
func (r *Repo) GetDeviceByID(ctx context.Context, id int) (*domain.Device, error) {
	return r.getDeviceByIDWithExec(ctx, r.db, id)
}

func (r *Repo) getDeviceByIDWithExec(ctx context.Context, exec sqlExecutor, id int) (*domain.Device, error) {
	if id <= 0 {
		return nil, nil
	}
...
func (r *Repo) DeleteByID(ctx context.Context, id int) error {
	return r.deleteByIDWithExec(ctx, r.db, id)
}

func (r *Repo) deleteByIDWithExec(ctx context.Context, exec sqlExecutor, id int) error {
	if id <= 0 {
		return sql.ErrNoRows
	}
	res, err := exec.ExecContext(ctx, `DELETE FROM devices WHERE id = $1`, id)
	if err != nil {
		return err
	}
...
func (r *Repo) UpdateDeviceByID(ctx context.Context, id int, patch *domain.Device) (*domain.Device, error) {
	return r.updateDeviceByIDWithExec(ctx, r.db, id, patch)
}

func (r *Repo) updateDeviceByIDWithExec(ctx context.Context, exec sqlExecutor, id int, patch *domain.Device) (*domain.Device, error) {
	if id <= 0 {
	...
	return r.getDeviceByIDWithExec(ctx, exec, id)
}

func (r *Repo) CreateDevice(ctx context.Context, device *domain.Device) error {
	return r.createDeviceWithExec(ctx, r.db, device)
}

func (r *Repo) createDeviceWithExec(ctx context.Context, exec sqlExecutor, device *domain.Device) error {
...
func (r *Repo) UpdateDeviceLastSeen(ctx context.Context, deviceID int) error {
	_, err := r.db.ExecContext(ctx, "UPDATE devices SET last_seen = NOW() WHERE id = $1", deviceID)
	return err
}
...
func (r *Repo) MarkDevicePollSuccess(ctx context.Context, deviceID int) error {
...
func (r *Repo) UpdateDeviceError(ctx context.Context, deviceID int, status, errText string) error {
