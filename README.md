...
- SMTP-конфиг (если включён) должен быть полным и валидным: `SMTP_HOST`, `SMTP_PORT`, `SMTP_FROM`; `SMTP_PORT` — число `1..65535`, в production допускаются только `465` (SMTPS) или `587` (STARTTLS).
- если используются `*_FILE` для `NMS_SESSION_SECRET`/`NMS_DB_ENCRYPTION_KEY`/`DB_DSN`, файлы должны существовать и быть непустыми; `NMS_TERMINAL_SSH_KNOWN_HOSTS` должен указывать на доступный файл.
