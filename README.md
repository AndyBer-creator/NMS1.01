...
2. **Миграции БД** (один раз, до первого запуска сервисов)
   ...
   Важно: для shared login rate limiter должна быть применена миграция `014_login_rate_limit_state.sql`, а для encrypted settings UI — `015_nms_settings_value_enc.sql` (обе входят в стандартный `make migrate` / `go run ./cmd/migration`).
