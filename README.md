...
1. **Секреты в .env**
   ...
   ```bash
   make init-secrets
   export NMS_SECRETS_DIR="${XDG_RUNTIME_DIR:-/tmp}/nms/secrets" # optional, default used by init-secrets
   ...
   `make init-secrets` копирует **содержимое** этого файла в `.secrets/nms_terminal_known_hosts`.
...
- `make init-secrets` создаёт `.secrets/nms_grpc_auth_token` из `NMS_GRPC_AUTH_TOKEN` (или fallback на `NMS_TRAP_GRPC_AUTH_TOKEN`).
...
- **unit** — тесты с **`-race`**, покрытие, порог **`scripts/check_coverage.sh`** (в CI сейчас **35%** через `MIN_COVERAGE_PERCENT=35`, локальный дефолт скрипта — 25%), загрузка в **Codecov** (ошибка загрузки не валит job), артефакт **`coverage-out`**; при push в ту же ветку предыдущий прогон этого workflow **отменяется** (`concurrency`);
