...
2. **Миграции БД** (один раз, до первого запуска сервисов)
   ...
   Важно: для shared login rate limiter должна быть применена миграция `014_login_rate_limit_state.sql` (она входит в стандартный `make migrate` / `go run ./cmd/migration`).
