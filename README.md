...
3. **Запуск стека**
   ...
   ```bash
   # после reboot/обычный запуск
   docker compose -f deploy/compose/docker-compose.yml up -d
   ```

   Миграции запускаются отдельно (one-off), при необходимости:

   ```bash
   docker compose -f deploy/compose/docker-compose.yml --profile migrate up migration
   ```

  По умолчанию **api** и **worker** в `network_mode: host` (весь стек в Docker, SNMP до LAN как с хоста — Linux или Docker Engine в WSL2). В `.env` пароль Postgres в `POSTGRES_PASSWORD` и в `DB_DSN` для migration/trap должен совпадать. `sslmode` для api/worker задаётся через `DB_SSLMODE` (default `disable` для локального single-host), для `postgres-exporter` — через `DB_SSLMODE_EXPORTER` (default `require`, для строгих сред лучше `verify-full`).
