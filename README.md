...
1. **Секреты в .env**
   ...
   ```bash
   make init-secrets
   export NMS_SECRETS_DIR="${XDG_RUNTIME_DIR:-/tmp}/nms/secrets" # required for docker-compose.secrets.yml
   ...
   Примечание по terminal SSH host keys: если в `.env` задан `NMS_TERMINAL_SSH_KNOWN_HOSTS` как путь к локальному файлу,
   `make init-secrets` копирует **содержимое** этого файла в `${NMS_SECRETS_DIR}/nms_terminal_known_hosts`.
   Если файл не найден, значение записывается как строка (legacy fallback), что обычно не подходит для production.
...
2. **Миграции БД** (один раз, до первого запуска сервисов)
  ...
  Важно: для shared login rate limiter должна быть применена миграция `014_login_rate_limit_state.sql`, для encrypted settings UI — `015_nms_settings_value_enc.sql`, и для hardening партиционированных метрик — `016_metrics_partition_hardening.sql` (все входят в стандартный `make migrate` / `go run ./cmd/migration`).
  Для legacy-данных метрик используется двухэтапный rollout: массовый backfill вынесен из `011` в controlled job `make metrics-backfill-legacy`, а финализация (drop `metrics_legacy`) — `make metrics-backfill-legacy-finalize` после проверки, что remaining=0.
   Если миграция не применена или БД недоступна, логин использует локальный in-memory fallback limiter.
...
3. **Запуск стека**
  ...
  По умолчанию **api** и **worker** в `network_mode: host` (весь стек в Docker, SNMP до LAN как с хоста — Linux или Docker Engine в WSL2). В `.env` пароль Postgres в `POSTGRES_PASSWORD` и в `DB_DSN` для migration/trap должен совпадать. `sslmode` для api/worker задаётся через `DB_SSLMODE` (default `disable` для локального single-host), для `postgres-exporter` — через `DB_SSLMODE_EXPORTER` (default `disable`); для production используйте минимум `require` (лучше `verify-full`).

   Если **Docker Desktop для Windows** не поднимает сервисы с `network_mode: host`, используйте полностью bridge-стек:

   ```bash
   docker compose -f deploy/compose/docker-compose.bridge.yml up -d
   ```
   ...
   **Политика развёртывания (production):** типовая площадка — выделенный **Linux в management VLAN** и основной `deploy/compose/docker-compose.yml` (**`network_mode: host`** для api/worker, чтобы SNMP/UDP вели себя как у процесса на хосте). `docker-compose.bridge.yml` — **альтернатива** для сред без нормальной поддержки host network, стендов или когда весь стек должен жить в userland Docker; **не поднимайте bridge в prod по умолчанию**, пока явно не проверили, что SNMP и трапы стабильно доходят до оборудования из bridge (см. про mirrored networking выше).

   В основном compose **Traefik** не проксирует api по Docker labels (api в host-сети); вход — **http://localhost:8080**. В `deploy/compose/docker-compose.bridge.yml` labels Traefik для api сохранены.

  Production profile (строгие overrides поверх основного compose):

  ```bash
  # Рекомендуемо вместе с secrets overlay
  docker compose -f deploy/compose/docker-compose.yml -f deploy/compose/docker-compose.prod.yml -f deploy/compose/docker-compose.secrets.yml up -d
  ```

  `deploy/compose/docker-compose.prod.yml` включает production-safe defaults:
  - `NMS_ENV=production`;
...
- **config.yaml** — HTTP-порт, SNMP-таймауты (без секретов). Если `config.yaml` присутствует и содержит синтаксические ошибки, сервис стартует с ошибкой (fail-fast), а не молча с дефолтами.
- **.env** — секреты: `DB_DSN`, `POSTGRES_PASSWORD`, `TELEGRAM_TOKEN`, `TELEGRAM_CHAT_ID`, `GRAFANA_ADMIN_PASSWORD`. См. `.env.example`.
- **deploy/compose/docker-compose.secrets.yml** — overlay для чтения секретов через `*_FILE` из `/.secrets` (рекомендуется для production).
...
- для non-production окружений можно включить те же строгие проверки через `NMS_REQUIRE_PROD_GUARDRAILS=true`.

### gRPC trap ingest security (token + TLS/mTLS)

gRPC-канал `trap-receiver -> api` поддерживает три режима:

- **Dev/pilot (по умолчанию):** без TLS и без токена.
- **Рекомендуемый минимум:** bearer-токен (`NMS_GRPC_AUTH_TOKEN` на `api` и `NMS_TRAP_GRPC_AUTH_TOKEN` на `trap-receiver`).
- **Production:** TLS (и при необходимости mTLS) + bearer-токен.

Базовые переменные:

- `NMS_GRPC_ADDR` — адрес gRPC listener у `api`.
- `NMS_TRAP_GRPC_TARGET` — gRPC target у `trap-receiver`.
- `NMS_GRPC_ALLOW_PUBLIC_BIND` — по умолчанию `false`; при `:9091`/`0.0.0.0`/`::` сервер не стартует, пока не выставите `true` явно.
- `NMS_GRPC_ALLOW_INSECURE_INGEST` — по умолчанию `false`; если не задан, gRPC ingest fail-closed требует и токен, и TLS. Для dev-режима без токена/TLS нужно явно `true`.

Token auth:

- `NMS_GRPC_AUTH_TOKEN` (или `NMS_GRPC_AUTH_TOKEN_FILE`) — ожидаемый токен на стороне `api`.
- `NMS_TRAP_GRPC_AUTH_TOKEN` (или `NMS_TRAP_GRPC_AUTH_TOKEN_FILE`) — токен, который `trap-receiver` отправляет в `authorization: Bearer ...`.
- `NMS_GRPC_AUTH_TOKEN_CACHE_TTL` — TTL кэша токена у `api` (default `2s`).
- `NMS_TRAP_GRPC_AUTH_TOKEN_CACHE_TTL` — TTL кэша токена у `trap-receiver` (default `2s`).

TLS/mTLS:

- `api` (server TLS):
  - `NMS_GRPC_TLS_CERT_FILE`
  - `NMS_GRPC_TLS_KEY_FILE`
  - `NMS_GRPC_TLS_CLIENT_CA_FILE` (опционально; включает проверку клиентского сертификата, то есть mTLS)
- `trap-receiver` (client TLS):
  - `NMS_TRAP_GRPC_TLS_CA_FILE`
  - `NMS_TRAP_GRPC_TLS_SERVER_NAME` (рекомендуется для корректной проверки SAN/CN)
  - `NMS_TRAP_GRPC_TLS_CERT_FILE` + `NMS_TRAP_GRPC_TLS_KEY_FILE` (опционально; клиентский сертификат для mTLS)

Compose/secrets:

- В `deploy/compose/docker-compose.secrets.yml` уже подключены `NMS_GRPC_AUTH_TOKEN_FILE` и `NMS_TRAP_GRPC_AUTH_TOKEN_FILE`.
- `make init-secrets` создаёт `${NMS_SECRETS_DIR}/nms_grpc_auth_token` из `NMS_GRPC_AUTH_TOKEN` (или fallback на `NMS_TRAP_GRPC_AUTH_TOKEN`).
- TLS-файлы сертификатов в overlay не включены принудительно; добавляйте их только когда реально подготовлены cert/key файлы.

Пример запуска (host profile) c token auth:

```bash
cp .env.example .env
# в .env задайте NMS_GRPC_ADDR=127.0.0.1:9091 и NMS_TRAP_GRPC_TARGET=127.0.0.1:9091
export NMS_GRPC_AUTH_TOKEN="$(openssl rand -hex 32)"
export NMS_TRAP_GRPC_AUTH_TOKEN="$NMS_GRPC_AUTH_TOKEN"
make init-secrets
docker compose -f deploy/compose/docker-compose.yml -f deploy/compose/docker-compose.secrets.yml up -d --build api trap-receiver
```

### Settings UI

- Runtime/secret настройки вынесены в отдельную страницу: `/settings/page`.
- На `/devices/list` оставлена ссылка на страницу настроек, чтобы не перегружать страницу устройств.
...
- Ручной restore: `RESTORE_CONFIRM_DROP=YES ./scripts/restore_postgres.sh <file.dump> <target_db>`
- `target_db` обязателен и должен быть отдельной restore/drill БД, не primary
- DR цели: `RPO <= 60m`, `RTO <= 120m`
- Offsite/immutable hooks:
  - `BACKUP_OFFSITE_SYNC_CMD`
  - `BACKUP_IMMUTABLE_COPY_CMD`
  - значения должны указывать на исполняемые скрипты (абсолютный/относительный путь), inline shell-команды не допускаются.
- Restore drill log:
  - `RESTORE_CONFIRM_DROP=YES RESTORE_DRILL_LOG=./logs/restore-drill.tsv`
- Подробный runbook: [`doc/BACKUP_RESTORE.md`](doc/BACKUP_RESTORE.md)

## Metrics Legacy Backfill (controlled rollout)

- `migrations/011_partition_metrics.sql` выполняет только DDL-переход на partitioned `metrics` и не делает массовый copy/drop legacy таблицы.
- Controlled backfill:
  - `make metrics-backfill-legacy`
  - optional throttling/safety flags: `go run ./cmd/backfill-metrics-legacy --batch-size 5000 --max-batches 100`
  - dry-run: `go run ./cmd/backfill-metrics-legacy --dry-run`
- Finalize step (только после `remaining=0`):
  - `make metrics-backfill-legacy-finalize`
...
- Alertmanager конфиг:
  - `deploy/monitoring/alertmanager.yml` (основной compose, webhook в `host.docker.internal:8080`)
  - `deploy/monitoring/alertmanager.bridge.yml` (bridge compose, webhook в `api:8080`)
  - при старте контейнера конфиг рендерится с bearer token из `NMS_ALERT_WEBHOOK_TOKEN` или `NMS_ALERT_WEBHOOK_TOKEN_FILE`, чтобы `POST /alerts/webhook` совпадал с auth-политикой API.
- Поток уведомлений:
...
Дополнительно после ротации gRPC токена:

```bash
make grpc-auth-sync-check
```

Проверка подтверждает, что у `api` и `trap-receiver` есть общий валидный источник токена:
- DB setting `grpc_auth_token_secret`, или
- fallback `.secrets/nms_grpc_auth_token`.
...
- Валидация Prometheus alert rules: `make alert-rules-check` (`promtool check rules` + `promtool test rules`; используется локальный `promtool` или Docker fallback).
...
- Для локальной отладки можно временно открыть `9090` через override:
  ...
  ```bash
  docker compose -f deploy/compose/docker-compose.yml -f deploy/compose/docker-compose.prom-host.yml up -d prometheus
  make slo-gates
  ```

  `deploy/compose/docker-compose.prom-host.yml` использовать только для dev/debug.
...
- Безопасный сценарий (по умолчанию): `make chaos-worker-check`
  - принудительно завершает процесс `worker` (`SIGKILL`) и проверяет восстановление сервиса + доступность worker metrics (с автоперезапуском или reconcile через `docker compose -f deploy/compose/docker-compose.yml up -d worker`).
...
- **Где запущен API** — в основном `deploy/compose/docker-compose.yml` api/worker уже в **host-сети** (всё в Docker). На **bridge** (`deploy/compose/docker-compose.bridge.yml`) включите в Docker Desktop **mirrored networking** или используйте Linux/WSL2 с основным compose.
...
**Основной** `deploy/compose/docker-compose.yml`: **api** и **worker** в **`network_mode: host`** — весь стек остаётся в Docker, а исходящий SNMP к вашей LAN (например `192.168.0.100`) идёт с **сетевого стека хоста** (удобно на **Linux** и при **Docker Engine в WSL2**). Подключение к Postgres у api/worker задаётся в compose: `127.0.0.1:5432`, пароль из `POSTGRES_PASSWORD` (должен совпадать с паролем в `DB_DSN` для migration/trap), `sslmode` из `DB_SSLMODE`. Для production включите TLS (`DB_SSLMODE=require` или `verify-full`). `deploy/monitoring/prometheus.yml` скрейпит метрики с `host.docker.internal:8080` и `:8081`.
...
Файл `deploy/compose/docker-compose.bridge.yml` монтирует **`deploy/monitoring/prometheus.bridge.yml`** (targets `api:8080`, `worker:8081`). Для доступа SNMP к LAN в этом режиме на Windows включите **mirrored networking** в настройках Docker Desktop (Resources → Network), иначе симптомы те же: **failed** в UI и пустой discovery при рабочем SNMP с хоста.
...
- **compose-security** — policy-check для `docker-compose*.yml` и `Dockerfile` (запрещены `privileged: true`, `:latest`, `pid/ipc: host`, unconfined `security_opt`, опасные capabilities и небезопасная публикация management-портов);
- **alert-rules** — проверка `alerts/nms-alerts.yml` через `promtool check rules` + rule-unit-tests `alerts/nms-alerts.test.yml` (локальный `promtool` или Docker fallback; обязательный CI gate);
...
- **unit** — тесты с **`-race`**, покрытие, порог **`scripts/check_coverage.sh`** (в CI сейчас **40%** через `MIN_COVERAGE_PERCENT=40`, локальный дефолт скрипта — 25%), загрузка в **Codecov** (ошибка загрузки не валит job), артефакт **`coverage-out`**; при push в ту же ветку предыдущий прогон этого workflow **отменяется** (`concurrency`);
...
Логи **worker** и **trap-receiver**: каталог **`NMS_LOG_DIR`** переопределяет путь; иначе в Docker (`NMS_ENV=docker`) — `/app/logs`, локально — `./logs`.
